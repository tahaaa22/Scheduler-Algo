...
public:
    ...
    Process* getTerminated() //getter for terminating running processes
    {
        return Terminate;
    }
   
    void addtoBLK(Process* process)  // setter for process to block 
    {
        BLK = process;
    }
    Process* getBLK() // getter for blocked processes
    {
        return BLK;
    }

    //virtual void addtoBLK(Process*& process)  // maybe phase 2 not sure yet
    //{
    //    if (getCurrRun() != NULL)
    //    {
   ...
   virtual void addToReadyQueue(Process *process) = 0;  // processor or scheduler
    ...
    ///*  bool isBusy() // fix in phase 2 ya taha -
    //  {
